...
//------------------------------------------------------------------------------
        ...
        var _generateControls = function(){
            ...
            if (me.$options.customControls !== false) {
                for (var i = 0; i < me.$options.customControls.length; ++i) {
                    menu.append(_generateCustomControl(me.$options.customControls[i]));
                }//for
            }
        ...
        var _generateCustomControl = function (controlOptions) {
            var control = $('<a data-func="' + controlOptions.func + '"></a>');
            control.append('<i class="' + LobiPanel.PRIVATE_OPTIONS.iconClass + ' ' + controlOptions.icon + '"></i>');
            if (controlOptions.tooltip && typeof controlOptions.tooltip === 'string') {
                control.append('<span class="control-title">'+controlOptions.tooltip+'</span>');
                control.attr('data-tooltip', controlOptions.tooltip);
            }
            control.on('click', controlOptions.click);
            return $('<li id="' + controlOptions.id + '"></li>').append(control);
        };
        ...
        var _addCustomControl = function(controlOption) {

        };

        var _addCustomControls = function(options) {
...
//                    bottom: $(window).height() - top - height + 2
            ...
            //we give .panel-body to width and height in order .panel-body to start scrolling
